...
{
		struct user_online *p=head1;
		FILE *fp=fopen("link.txt","w+");
		while(p!=NULL)
		{
			fwrite(p,sizeof(struct user_online),1,fp);
			p=p->next;
		}
			fclose(fp);
}
...
void show_user_online()
{
		...
		while(p!=NULL)
		{
			...
			//		printf("please input passenger's id(0 exit,1 refresh):");
				}
				p=p->next;
		}
}
